...
Function ConfigureApplications
{
   ...
   Write-Host "Creating the AAD application (WebApp-blazor-wasm)"
   # create the application 
   $webAppAadApplication = New-MgApplication -DisplayName "WebApp-blazor-wasm" `
                                                      -Spa `
                                                      @{ `
    ...
    #Update-MgApplication -ApplicationId $currentAppObjectId -IdentifierUris @("https://$tenantName/WebApp-blazor-wasm")
    ...
    Write-Host "Done creating the webApp application (WebApp-blazor-wasm)"
    ...
    Add-Content -Value "<tr><td>webApp</td><td>$currentAppId</td><td><a href='$webAppPortalUrl'>WebApp-blazor-wasm</a></td></tr>" -Path createdApps.html
    ...
    Write-Host "Successfully registered and configured that app registration for 'WebApp-blazor-wasm' at `n $webAppPortalUrl" -ForegroundColor Green 
    
    # Update config file for 'webApp'
    # $configFile = $pwd.Path + "\..\blazorwasm-calls-MS-graph\wwwroot\appsettings.json"
    $configFile = $(Resolve-Path ($pwd.Path + "\..\blazorwasm-calls-MS-graph\wwwroot\appsettings.json"))
    
    $dictionary = @{ "Enter_Application_ID_here" = $webAppAadApplication.AppId;"Enter_Tenant_ID_here" = 'https://login.microsoftonline.com/'+$tenantId };
